...
import { normalizeString, getRealDOMId } from 'lightning/utilsPrivate';
import { classSet } from 'lightning/utils';
import { Tooltip, TooltipType, BUBBLE_PREFIX } from 'lightning/tooltipLibrary';
...
class LightningHelptext extends LightningElement {
  ...
  /**
   * override the aria-describedby to link to the DOM element present
   * inside the helptext button. We are doing this because, screen readers
   * can't read tooltip content which is present outside modal.
   */


  overrideDescribedBy() {
    const domId = getRealDOMId(this.template.querySelector(SELECTORS.DESCRIPTION));
    this.template.querySelector(SELECTORS.BUTTON).setAttribute('aria-describedby', domId);
  }
  /**
   * use the bubble prefix format from tooltip library
   * @returns {*}
   */


  get describedByID() {
    return BUBBLE_PREFIX;
  }
